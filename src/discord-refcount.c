...
_on_ref_map_changed(const struct anomap_item_changed *ev)
{
    ...
    if (ev->op & anomap_delete) {
        ...
        logconf_info(&rc->conf, "deleting %s %p", val->name,
                     *(void **)ev->key);
                                ...
                                int claims)
{
    ...
    if (found) {
        struct _discord_refvalue *val = anomap_direct_val_at_index(map, index);

        if ((val->visits += visits) < 0)
            logconf_info(&rc->conf, "visits for %p below 0", data);

        if ((val->claims += claims) < 0)
            logconf_info(&rc->conf, "claims for %p below 0", data);

        if (visits)
            logconf_info(&rc->conf, "visits %p %s by %i to %i", data,
                         visits > 0 ? "incrementing" : "decrementing",
                         visits > 0 ? visits : -visits, val->visits);
        if (claims)
            logconf_info(&rc->conf, "claims %p %s by %i to %i", data,
                         claims > 0 ? "incrementing" : "decrementing",
                         claims > 0 ? claims : -claims, val->claims);

        if (0 == val->visits && 0 == val->claims) {
            anomap_do(map, anomap_delete, &data, NULL);
            code = CCORD_OK;
        }
        else if (val->claims >= 0 && val->visits >= 0) {
            code = CCORD_OK;
        }
    }
    else {
                                ...
                                bool should_free)
{
    ...
    enum anomap_operation result = anomap_do(map, anomap_insert, &data, &val);
    logconf_info(&rc->conf, "Adding concord's internal resource %s %p %s",
                 name, data, result & anomap_insert ? "successful" : "failed");
    pthread_mutex_unlock(&rc->locks[arrayi]);
}
                              ...
                              bool should_free)
{
    ...
    enum anomap_operation result = anomap_do(map, anomap_insert, &data, &val);
    logconf_info(&rc->conf, "Adding user's custom resource %s %p %s", name,
